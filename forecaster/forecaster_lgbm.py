...
def train_model(items, X_train, y_train, X_val, y_val, X_test, params=None, maxRounds=5000):
    if params is None:
        params = {
            'num_leaves': 2**5 - 1,
    ...
    for i in range(FutureDaysToCalculate):
        ...
        val_pred.append(bst.predict(
            X_val, num_iteration=bst.best_iteration or maxRounds))
        test_pred.append(bst.predict(
            X_test, num_iteration=bst.best_iteration or maxRounds))

    rmse = np.sqrt (mean_squared_error(np.expm1(y_val), np.expm1(np.array(val_pred)).transpose()))

    return val_pred, test_pred, rmse


if __name__ == "__main__":
    (loc, min_date, cumul_sale_tbl, cumul_sale_future_tbl, items_tbl, stores_tbl) = sys.argv[1:]
    if loc == "csv":
        cumul_sales, cumul_sales_future, items, stores = \
    ...
    X_train, y_train, X_val, y_val, X_test = create_machine_learning_matrices(\
    ...
    val_pred, test_pred, rmse = train_model(items, X_train, y_train, X_val, y_val, X_test)
    print ("Validation error is :" + str(rmse))
